# Program Name: summarize-friedman-sim.R
# Description:  Summarize results of Friedman simulation.
...
# Load results
results <- readRDS(here('friedman-sim','Results','friedman-sim-results.rds'))
...
ggsave(here('friedman-sim','Figures','ext-friedman-sim-var-imp.png'),
...
var_imp |>
  ...
  labs(x = 'Covariate Index',
       y = 'Proportion of Splits',
       color = '# of Trees')
ggsave(here('friedman-sim','Figures','friedman-sim-var-imp.png'), 
...
var_imp |>
  ...
  labs(x = 'Covariate',
       ...
       color = 'Number of Trees')
ggsave(here('friedman-sim','Figures','ext-friedman-sim-psp.png'),
       width = 6.5, height = 4.5)
...
var_imp |>
  ...
  labs(x = 'Covariate Index',
       ...
       color = 'Number of Trees')
ggsave(here('friedman-sim','Figures','ext-friedman-sim-pip.png'), 
       width = 6.5, height = 4.5)
...
bart_stats_tbl <- bart_stats_summary[,2:9] |>
  kbl(format = 'latex', booktabs = TRUE, linesep = '', escape = FALSE,
      align = c('c'), label = 'ext-friedman-sim-bart-stats',
      caption = 'Extended Friedman Simulation Results - BART Predictions',
      col.names = c('Type',
...
write_lines(bart_stats_tbl, 
            here('friedman-sim','Tables','ext-friedman-sim-bart-stats.txt'))
...
bart_stats_tbl_sub <- bart_stats_summary |>
  ...
  kbl(format = 'latex', booktabs = TRUE, linesep = '', escape = FALSE,
      align = c('c'), label = 'friedman-sim-bart-stats',
      caption = 'Friedman Simulation Results - BART Predictions',
      col.names = c('Type',
                    paste0('$M$', footnote_marker_alphabet(1, 'latex')),
...
write_lines(bart_stats_tbl_sub, 
            here('friedman-sim','Tables','friedman-sim-bart-stats.txt'))
...
beta_stats_tbl_bias <- beta_stats_summary |>
  ...
  kbl(format = 'latex', booktabs = TRUE, linesep = '', escape = FALSE,
      align = c('c'), label = 'ext-friedman-sim-beta-bias',
      caption = 'Extended Friedman Simulation Results - Confounder Estimates (Bias)',
      col.names = c('Type',
                    paste0('$M$', footnote_marker_alphabet(1, 'latex')),
                    paste0('$k$', footnote_marker_alphabet(2, 'latex')),
                    paste0('$(\\gamma, \\xi)$', footnote_marker_alphabet(3, 'latex')),
...
beta_stats_tbl_coverage <- beta_stats_summary |>
  ...
  kbl(format = 'latex', booktabs = TRUE, linesep = '', escape = FALSE,
      align = c('c'), label = 'ext-friedman-sim-beta-coverage',
      caption = 'Extended Friedman Simulation Results - Confounder Estimates (Coverage)',
      col.names = c('Type',
                    paste0('$M$', footnote_marker_alphabet(1, 'latex')),
                    paste0('$k$', footnote_marker_alphabet(2, 'latex')),
                    paste0('$(\\gamma, \\xi)$', footnote_marker_alphabet(3, 'latex')),
...
write_lines(beta_stats_tbl_bias, 
            here('friedman-sim','Tables','ext-friedman-sim-beta-bias.txt'))
write_lines(beta_stats_tbl_coverage, 
            here('friedman-sim','Tables','ext-friedman-sim-beta-coverage.txt'))
...
waic |>
  ...
  scale_linetype_manual(values = c('BART' = 1, 'DART' = 2)) +
  theme_bw() +
  theme(legend.position = 'inside',
  ...
  labs(x = 'Number of Trees',
       y = 'Relative WAIC')
ggsave(here('friedman-sim','Figures','ext-friedman-sim-waic.png'), 
       width = 6.5, height = 4.5)
...
ggsave(here('friedman-sim','Figures','ext-friedman-sim-time.png'), 
       width = 6.5, height = 4.5)


# Partial Dependence ------------------------------------------------------

# Load marginal partial dependence statistics
marg_pd <- bind_rows(lapply(results, '[[', 'marg_pd')) |>
  left_join(params, by = join_by(key)) |>
  filter(key %in% keys & num_tree == 100)

# Plot the "average" pointwise partial dependence function across simulations
marg_pd |>  
  mutate(x = round(x, 1)) |>
  summarise(lcl = quantile(est, 0.025), # 2.5th %ile of curve, pointwise
            ucl = quantile(est, 0.975), # 97.5th %ile of curve, pointwise
            est = mean(est),
            truth = mean(truth),
            .by = c(x, var, key)) |>
  ...
  geom_line(aes(y = truth, color = 'Truth'), lty = 2) +
  facet_wrap(~factor(var, levels = unique(marg_pd$var)), nrow = 2, scales = 'free') +
  theme_bw() +
  scale_x_continuous(breaks = seq(0,1,0.5)) +
  theme(legend.position = 'bottom',
        text = element_text(size = 13)) +
  scale_color_manual(values = c("Truth" = 2, "Estimate" = 1)) +
  labs(x = 'Covariate Value',
       y = 'Partial Dependence',
       color = '')  +
  guides(color = guide_legend(override.aes = list(lty = c(1, 2))))
ggsave(here('friedman-sim','Figures','friedman-sim-pdp.png'), 
       width = 6.5, height = 5)
