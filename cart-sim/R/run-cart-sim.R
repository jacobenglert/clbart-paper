# Program Name: run-cart-sim.R
# Description:  Simulate log odds-ratios from a simple "CART" truth and
#               attempt to recover using CL-BART.
...
# Modifiers
rho <- 0.6
w_cor <- sapply(1:p_w, \(x) rho^(abs(x - (1:p_w))))
w <- data.frame(matrix(as.numeric(rmvnorm(n, sigma = w_cor) > 0), nrow = n))

# Shuffle columns of w
w <- w[,sample(1:p_w)]
...
split_props[is.nan(split_props)] <- 0
...
cart_data <- cbind(w, tau = fit$lambda_est)[firsts,]
cart      <- rpart(tau ~ ., data = cart_data)
cart_vars <- names(cart$variable.importance[1:3])

# Compute R-squared for lower-dimensional summary
cart_R2 <- 1 - (sum((cart_data$tau - predict(cart))^2) / 
                  sum((cart_data$tau - mean(cart_data$tau))^2))
...
tau_pd_est <- left_join(w, cart_pd, by = join_by(!!!syms(cart_vars)))$est
tau_pd_lcl <- left_join(w, cart_pd, by = join_by(!!!syms(cart_vars)))$lcl
tau_pd_ucl <- left_join(w, cart_pd, by = join_by(!!!syms(cart_vars)))$ucl

# Bias, rmse, coverage and interval width
avg_bias_pd      <- mean(tau_pd_est - tau)
rmse_pd          <- sqrt(mean((tau_pd_est - tau)^2))
avg_coverage_pd  <- mean(between(tau, tau_pd_lcl, tau_pd_ucl))
avg_width_pd     <- mean(tau_pd_ucl - tau_pd_lcl)

cart_pd_stats <- data.frame(type = 'clbart-pd',
                            key = key, seed = seed, 
                            avg_bias = avg_bias_pd, 
                            rmse = rmse_pd, 
                            avg_coverage = avg_coverage_pd, 
                            avg_width = avg_width_pd)


# Fit and Calculate Statistics for Oracle Model ---------------------------

# True moderator design matrix
w_true <- cbind(w[,1] <= 0 & w[,2] <= 0,
                w[,1] <= 0 & w[,2] > 0,
                w[,1] > 0 & w[,3] <= 0,
                w[,1] > 0 & w[,3] > 0) |>
  as.numeric() |>
...
oracle_se <- sqrt(diag(oracle$var)[coef_idx])

# Point and interval estimates for BART predictions
tau_est_oracle <- as.numeric(w_true %*% oracle_pt)
tau_lcl_oracle <- as.numeric(w_true %*% (oracle_pt - 1.96*oracle_se))
tau_ucl_oracle <- as.numeric(w_true %*% (oracle_pt + 1.96*oracle_se))
...
results <- list(key = key, seed = seed, waic = fit$WAIC, time = fit$time,
                ...
                bart_stats = rbind(bart_stats, cart_pd_stats, bart_stats_oracle),
                var_imp = var_imp, 
                cart_vars = cart_vars)
ID <- paste0(sprintf('%03d', key), '-', sprintf('%03d', seed))
saveRDS(results, here('cart-sim','Results','temp', paste0(ID, '.rds')))
